...
import joblib, os, io, base64, warnings, numpy as np, csv
...
app = Flask(__name__)
app.secret_key = "ecopackai_prod_2026_btech_secure_v2"
MODEL_PATH = 'model'

# ---------------- MODEL LOADING ----------------
def load_models():
    try:
        rf_cost = joblib.load(os.path.join(MODEL_PATH, 'rf_cost.pkl'))
        rf_co2 = joblib.load(os.path.join(MODEL_PATH, 'rf_co2.pkl'))
        scaler = joblib.load(os.path.join(MODEL_PATH, 'scaler.pkl'))
        features = joblib.load(os.path.join(MODEL_PATH, 'feature_columns.pkl'))
        return rf_cost, rf_co2, scaler, features
    except:
        return None, None, None, None

rf_cost, rf_co2, scaler, feature_columns = load_models()
...
def login():
    if request.method=="POST":
        user = authenticate_user(request.form['username_or_email'], request.form['password'])
        if user:
...
def signup():
    if request.method=="POST":
        if create_user(request.form['username'], request.form['email'], request.form['password']):
...
def predict():
    ...
    if request.method=="POST":
        ...
        cur.execute("""
            ...
            (user_id, product_category, weight_g, fragility_level, dimensions_cm, strength,
             predicted_cost, predicted_co2, recommended_material)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (session["user_id"], product, weight, fragility, dimensions, strength, cost, co2, material))
        conn.commit()
        cur.close(); conn.close()
...
# ---------------- REST API (MILESTONE 3) ----------------
@app.route("/api/predict", methods=["POST"])
def api_predict():
    data = request.json
    material = get_recommendation(
        data["weight"], data["fragility"],
        data["dimensions"], data["strength"]
    )
    return jsonify({
        "recommended_material": material,
...
def export_csv():
    ...
    mem.seek(0)
    return send_file(mem, as_attachment=True, download_name="EcoPackAI_Report.csv", mimetype="text/csv")
